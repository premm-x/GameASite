import React, { useState } from 'react';
import user from "../assets/user.png"

const teamMembers = [
    {
        name: 'prem',
        memeberType: 'Team Leader',
        detail: 'sybca',
    },
    {
        name: 'aman',
        memeberType: 'mamber',
        detail: 'sybca',
    },
    {
        name: 'ajay',
        memeberType: 'mamber',
        detail: 'sybca',
    },
    {
        name: 'raju',
        memeberType: 'mamber',
        detail: 'sybca',
    },
    {
        name: 'baju',
        memeberType: 'mamber',
        detail: 'sybca',
    }
]

const Team = () => {

    const [userDetail, setUserDetail] = useState({name: 'prem',memeberType: 'Team Leader',detail: 'sybca',});

    const userHandler = (member) =>{
        setUserDetail(member)
    }

    return (
        <div className='text-white bg-black min-h-screen'>
           <div className='w-full flex items-center justify-center flex-col pt-10'>
                <h1 className='text-white font-semibold text-5xl'>Our Team</h1>
                <p className='text-white font-semibold text-center text-3xl mt-2'>Meet the amazing team behind our game.</p>
           </div>

           <div className='py-4 px-4 w-full mt-20 flex items-center justify-center md:flex-row flex-col-reverse gap-20'>

                <div className='md:w-[50%] w-[100%]'>
                    {teamMembers.map((member, idx)=>(
                        <div onClick={()=>{userHandler(member)}} key={idx} className='flex items-center w-full py-2 mb-2 px-4 border-2 border-gray-600 rounded-xl justify-start gap-8 bg-gray-900'>
                            <img className='bg-white rounded-full w-10' src={user} alt="" />
                            <div>
                                <p>{member.memeberType}</p>
                                <p className='ml-4'>- {member.name}</p>
                            </div>
                        </div>
                    ))}
                </div>
                

                <div className='bg-gray-800 rounded-xl md:h-96 h-72 md:w-[40%] w-[100%] flex items-center justify-between'>
                    <div className='w-[50%] h-full flex flex-col items-start justify-center gap-8 px-6'>
                        <p className='text-white font-semibold text-xl'>- {userDetail.memeberType}</p>
                        <p className='text-white font-semibold text-xl'>Name:- {userDetail.name}</p>
                        <p className='text-white font-semibold text-xl'>Class:- {userDetail.detail}</p>
                    </div>
                    <div className='w-[50%]  rounded-xl h-full bg-slate-400'>
                        <img className='w-full  rounded-xl h-full object-cover object-center' src="data:image/jpeg;base64," alt="" />
                    </div>
                </div>

           </div>

        </div>
    );
};

export default Team;